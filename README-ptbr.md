...
Um [relato independente sobre o AN16S-61](https://github.com/cleyton1986/predator-sense/issues/31) (veja também a [ferramenta de protocolo standalone](https://github.com/ArnarValur/Nitro16S-AI-RGB-Keyboard) do próprio autor) mapeou mais seis modos nativos além de estático/Respiração/Neon/Onda (um modo de desligar via hardware, um modo de pisca de boot que o próprio EC dispara, e mais quatro animações nativas), além de um alvo de LED da tecla de modo/turbo. Nada disso está ligado no app ainda — precisa de um slot definido pra código de efeito nativo de hardware antes de poder ser, então fica registrado como melhoria futura.

O mesmo relato também trouxe um report descriptor HID decodificado direto do controlador, que resolveu um bug real: o app lia a contagem de zonas do relatório de capacidades A3 do byte errado (`byte[3]`, uma constante fixa por classe de alvo) em vez do byte que o próprio descriptor do controlador declara pra isso (`byte[4]`). Corrigido na `v0.2.69-preview` no app e no daemon de hotkey. É uma correção de nível de protocolo, não uma mudança por modelo — o layout de campo do report descriptor vem do firmware do próprio chip (mesmo chip `0CF2:5130` nos três modelos confirmados) — e não muda nenhum byte de fio em hardware já confirmado funcionando, já que o valor anterior sempre foi um superconjunto inofensivo do valor certo.
